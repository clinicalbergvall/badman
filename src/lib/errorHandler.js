 ...
 */
export function createSanitizedError(error) {
    const message = sanitizeErrorMessage(error);
    // Determine if this is a user-friendly error
    const isUserFriendly = Object.values(USER_FRIENDLY_MESSAGES).some(friendlyMsg => message === friendlyMsg || message.includes(friendlyMsg));
    return {
        message,
        isUserFriendly,
    };
 ...
 */
export function containsSensitiveData(error) {
    if (!error)
        return false;
    let errorMessage = '';
    if (error instanceof Error) {
