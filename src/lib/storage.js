...
const BOOKING_KEY = "detailpro-booking";
const HISTORY_KEY = "detailpro-history";
const CLEANER_PROFILE_KEY = "detailpro-cleaner-profile";
const CLEANER_PENDING_KEY = "detailpro-pending-cleaners";
const CLEANER_APPROVED_KEY = "detailpro-approved-cleaners";
const CLEANER_JOB_OPPORTUNITIES_KEY = "detailpro-job-opportunities";
const CLEANER_CHECKLIST_KEY = "detailpro-job-checklist";
export const USER_SESSION_KEY = "detailpro-user-session";
...
function saveCleanerJobOpportunities(jobs) {
    try {
...
function withDefaultJobState(jobs) {
    return jobs.map((job) => ({ ...job, saved: job.saved ?? false }));
}
export async function refreshCleanerJobOpportunities() {
    try {
...
export function loadCleanerJobOpportunities() {
    try {
...
export function toggleSavedJob(jobId) {
    ...
    const updated = jobs.map((job) => job.id === jobId ? { ...job, saved: !job.saved } : job);
    saveCleanerJobOpportunities(updated);
    return updated;
}
export function addBookingToCleanerJobs(booking) {
    const jobs = loadCleanerJobOpportunities();
    const id = booking.id || `booking-${Date.now()}`;
    const titleParts = [booking.carServicePackage, booking.vehicleType].filter(Boolean);
    const cleanerPayout = Math.round((booking.price || 0) * 0.4);
    const opportunity = {
        id,
        bookingId: booking.id,
        title: titleParts.join(" · ") || "Car Detailing Job",
        location: booking.location?.manualAddress ||
            booking.location?.address ||
            "Client to confirm exact location",
        payout: currencyFormatter.format(cleanerPayout),
        timing: booking.bookingType === "scheduled" && booking.scheduledDate
        ...
        priority: (booking.price || 0) >= JOB_PRIORITY_THRESHOLD ? "featured" : "standard",
        saved: false,
...
function saveCleanerChecklist(items) {
    try {
...
export function loadCleanerChecklist() {
    try {
...
export function saveCurrentBooking(booking) {
    try {
...
export function loadCurrentBooking() {
    try {
...
export function saveCleanerProfile(profile) {
    try {
...
export function loadCleanerProfile() {
    try {
...
export function clearCleanerProfile() {
    try {
...
function savePendingCleaners(cleaners) {
    try {
...
export function loadPendingCleaners() {
    try {
...
export function addPendingCleaner(profile) {
...
export function removePendingCleaner(id) {
    const cleaners = loadPendingCleaners().filter((c) => c.id !== id);
    savePendingCleaners(cleaners);
}
function saveApprovedCleaners(cleaners) {
    try {
...
export function loadApprovedCleaners() {
    try {
...
export function addApprovedCleaner(profile) {
...
export function updateStoredCleanerProfile(id, updates) {
...
export const setupSessionSync = (onSessionChange) => {
