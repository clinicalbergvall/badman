        ...
        if (Capacitor.isNativePlatform()) {
            try {
                ...
                if (Geolocation) {
                    ...
                    const fine = status?.location || status?.coarseLocation;
        ...
        if (Capacitor.isNativePlatform()) {
            try {
                ...
                if (Geolocation) {
                    ...
                    const fine = result?.location || result?.coarseLocation;
                    return fine === 'granted';
                }
            }
            catch (error) {
...
export async function reverseGeocode(lat, lng) {
    try {
        ...
        const response = await fetch(`https://maps.googleapis.com/maps/api/geocode/json?latlng=${lat},${lng}&key=${apiKey}&result_type=street_address`);
    ...
    try {
        ...
        if (Capacitor.isNativePlatform()) {
            ...
            if (Geolocation) {
                ...
                const watchIdObject = await Geolocation.watchPosition(async (position) => {
                    if (position) {
                        const location = {
                            latitude: position.coords.latitude,
                            longitude: position.coords.longitude,
