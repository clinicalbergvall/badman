...
export const CAR_SERVICE_PACKAGES = [
    {
        id: "NORMAL-DETAIL",
        name: "Normal Detail",
    ...
    },
    {
        ...
        description: "Sanitation, deep cleaning and removal of stains & odours from the car",
        duration: "2-3 hours",
    },
    {
        id: "PAINT-CORRECTION",
    ...
    },
    {
        id: "FULL-DETAIL",
        name: "Full Detail",
        description: "Complete detailing package - interior, exterior, and protection",
        duration: "4-6 hours",
    },
    {
        id: "FLEET-PACKAGE",
...
export const PAINT_CORRECTION_STAGES = [
    ...
    },
    {
        id: "STAGE-3",
        name: "Stage 3",
        description: "Heavy paint correction - removes deep scratches and oxidation",
    },
];
export const CAR_DETAILING_EXTRAS = [
    {
...
export const CAR_DETAILING_PRICING = {
    "NORMAL-DETAIL": {
        SEDAN: 7000,
...
export function getCarDetailingPrice(vehicleType, servicePackage, paintStage, midSUVTier, fleetCarCount) {
    let priceKey = servicePackage;
    if (servicePackage === "PAINT-CORRECTION" && paintStage) {
        priceKey = `PAINT-CORRECTION-${paintStage}`;
    }
    ...
    if (servicePackage === "FLEET-PACKAGE" && fleetCarCount) {
        return basePrice * fleetCarCount;
    }
    return basePrice;
}
...
export function getCarServicePackage(id) {
    return CAR_SERVICE_PACKAGES.find((pkg) => pkg.id === id);
}
export function getPaintCorrectionStage(id) {
    return PAINT_CORRECTION_STAGES.find((stage) => stage.id === id);
}
export function getCarDetailingExtra(id) {
...
export function getHomeCleaningPrice(category, details) {
    let total = 0;
    if (category === "HOUSE_CLEANING") {
        if (details.houseCleaningType === "BATHROOM" && details.bathroomItems) {
            if (details.bathroomItems.general)
                total += BATHROOM_PRICING.GENERAL;
            if (details.bathroomItems.sink)
                total += BATHROOM_PRICING.SINK;
            if (details.bathroomItems.toilet)
                total += BATHROOM_PRICING.TOILET;
        }
        ...
        details.roomSize) {
        const pricing = details.fumigationType === "GENERAL"
...
export const phoneSchema = z
    .string()
    .regex(/^(07|01)\d{8}$/, "Phone must be Kenyan format (07XXXXXXXX or 01XXXXXXXX)");
export const mpesaSchema = z
    .string()
    .regex(/^[A-Z0-9]{10}$/, "M-PESA code must be exactly 10 alphanumeric characters")
...
export const cleaningServicePackages = [];
export const CLEANING_SERVICE_PACKAGES = cleaningServicePackages;
export const homeCleaningAddons = [];
export const getCarPrice = getCarDetailingPrice;
export const getCleaningPrice = (propertySize, service) => {
    ...
    return getHomeCleaningPrice("HOUSE_CLEANING", {
        roomSize: propertySize,
        houseCleaningType: service
    });
};
export const getPropertyCategory = (id) => propertyCategories.find((c) => c.id === id);
export const getCleaningServicePackage = (id) => cleaningServicePackages.find((s) => s.id === id);
